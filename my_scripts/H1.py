...
class DiffrigidLocoConfig:
    dt: float = 0.01
    n_envs: int = 1           # start with 1 env to match the test style
    horizon: int = 64
    num_iter: int = 200
...
class GenesisH1DiffrigidEnv:
    def __init__(self, cfg: DiffrigidLocoConfig):
        ...
        self.scene = gs.Scene(
            ...
            rigid_options=gs.options.RigidOptions(
                enable_collision=False, #True,
                enable_self_collision=False,
                enable_joint_limit=False, #True,
                disable_constraint=False,
                use_contact_island=False, #True,
    ...
    def reset_scene(self):
        ...
        base_pos = gs.tensor([[0.0, 0.0, self.cfg.target_height]] * self.cfg.n_envs, requires_grad=False)
