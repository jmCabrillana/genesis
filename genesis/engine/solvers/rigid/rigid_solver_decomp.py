...
class RigidSolver(Solver):
    ...
    def update_qvel(self, f: ti.i32):
        ...
        for i_0, i_b in ti.ndrange(1, self._B) if ti.static(self._use_hibernation) else ti.ndrange(self.n_dofs, self._B):
...
def func_compute_mass_matrix(
    ...
    # crb initialize
    ti.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL)
    for i_0, i_b in ti.ndrange(1, _B) if ti.static(static_rigid_sim_config.use_hibernation) else ti.ndrange(n_links, _B):
        for i_1 in range(rigid_global_info.n_awake_links[i_b]) if ti.static(static_rigid_sim_config.use_hibernation) else range(1):
    ...
    # crb
    ti.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL)
    for i_0, i_b in ti.ndrange(1, _B) if ti.static(static_rigid_sim_config.use_hibernation) else ti.ndrange(n_entities, _B):
        for i_1 in range(rigid_global_info.n_awake_entities[i_b]) if ti.static(static_rigid_sim_config.use_hibernation) else range(1):
            i_e = rigid_global_info.awake_entities[i_1, i_b] if ti.static(static_rigid_sim_config.use_hibernation) else i_0
...
def func_solve_mass_batched(
    ...
    ti.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.PARTIAL)
    for i_0 in range(rigid_global_info.n_awake_entities[i_b]) if ti.static(static_rigid_sim_config.use_hibernation) else range(n_entities):
...
def func_COM_links(
    ...
    ti.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL)
    for i_l_ in range(rigid_global_info.n_awake_links[i_b]) if ti.static(static_rigid_sim_config.use_hibernation) else range(n_links):
    ...
    for i_l_ in range(rigid_global_info.n_awake_links[i_b]) if ti.static(static_rigid_sim_config.use_hibernation) else range(n_links):
        ...
        if i_l == i_r and links_state.mass_sum[f, i_l, i_b] > 0.0:
            links_state.root_COM[f, i_l, i_b] = (
                links_state.root_COM[f, i_l, i_b] / links_state.mass_sum[f, i_l, i_b]
            )
...
def func_update_geoms(
    ...
    ti.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.PARTIAL)
    for i_0 in range(rigid_global_info.n_awake_entities[i_b]) if ti.static(static_rigid_sim_config.use_hibernation) else range(n_geoms):
        i_e = rigid_global_info.awake_entities[i_0, i_b] if ti.static(static_rigid_sim_config.use_hibernation) else 0
        
        for i_1 in range(entities_info.geom_start[i_e], entities_info.geom_end[i_e]) if ti.static(static_rigid_sim_config.use_hibernation) else range(1):
...
def func_clear_external_force(
    ...
    ti.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.PARTIAL)
    for i_f, i_0, i_b in ti.ndrange(n_frames, 1, _B) if ti.static(static_rigid_sim_config.use_hibernation) else ti.ndrange(n_frames, n_links, _B):
        for i_1 in range(rigid_global_info.n_awake_links[i_b]) if ti.static(static_rigid_sim_config.use_hibernation) else range(1):
            i_l = rigid_global_info.awake_links[i_1, i_b] if ti.static(static_rigid_sim_config.use_hibernation) else i_0
...
def func_torque_and_passive_force(
    ...
    for i_0, i_b in ti.ndrange(1, _B) if ti.static(static_rigid_sim_config.use_hibernation) else ti.ndrange(n_dofs, _B):
        for i_1 in range(rigid_global_info.n_awake_dofs[i_b]) if ti.static(static_rigid_sim_config.use_hibernation) else range(1):
        ...
        for i_l in range(entities_info.link_start[i_e], entities_info.link_end[i_e]):
            ...
            if joint_type == gs.JOINT_TYPE.FIXED:
                ...
                for i_d_ in range(links_info.dof_end[I_l] - dof_start):
                    i_q = q_start + i_d_
                    i_d = dof_start + i_d_
                    rigid_global_info.qpos[i_f, i_q, i_b] = rigid_global_info.qpos0[i_q, i_b] + dofs_state.pos[i_f, i_d, i_b]
