...
for i in bar:
    ...
    with th.no_grad():
        curr_input_mass = input_mass.detach().numpy()
        curr_input_jac = input_jac.detach().numpy()
        curr_input_aref = input_aref.detach().numpy()
        curr_input_efc_D = input_efc_D.detach().numpy()
        curr_input_force = input_force.detach().numpy()
        curr_input_acc_smooth = input_acc_smooth.detach().numpy()
    ...
    loss = th_output_qacc.mean()  # (th_output_qacc ** 2).mean()
    ...
    with th.no_grad():
        input_mass.data = init_input_mass
        input_jac.data = init_input_jac
        input_aref.data = init_input_aref
        input_efc_D.data = init_input_efc_D
