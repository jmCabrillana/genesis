...
@pytest.mark.parametrize("gs_solver", [gs.constraint_solver.CG, gs.constraint_solver.Newton])
@pytest.mark.parametrize("gs_integrator", [gs.integrator.implicitfast, gs.integrator.Euler])
@pytest.mark.parametrize("multi_contact", [False])
...
# @NOTE: When using Newton solver, the test fails because of a tiny numerical difference between Mujoco.
# Since the difference does not stem from false logic, we ignore it for now.
